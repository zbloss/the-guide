...
export function SessionDetailPage() {
  ...
  const [tab, setTab] = useState<
    "events" | "summary" | "debrief" | "loot" | "map" | "voice"
  >("events");
  ...
  const handleGenerateSummary = async () => {
    ...
    setSummary(null);
    try {
      const result = await getSessionSummary(campaignId!, sessionId!);
      setSummary(result);
    } catch (e: unknown) {
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <Link to={`/campaigns/${campaignId}/sessions`} className="breadcrumb">
            ← Sessions
          </Link>
          <h1>
        ...
        <div className="session-actions">
          ...
          {deriveSessionStatus(session) === "ended" && (
            <span className="badge badge-default">Ended</span>
          )}
        </div>
      </div>
      ...
      <div className="tab-nav">
        <button
          className={`tab-link ${tab === "events" ? "active" : ""}`}
          onClick={() => setTab("events")}
        >
          Events
        </button>
        <button
          className={`tab-link ${tab === "summary" ? "active" : ""}`}
          onClick={() => setTab("summary")}
        >
          Summary
        </button>
        <button
          className={`tab-link ${tab === "debrief" ? "active" : ""}`}
          onClick={() => setTab("debrief")}
        >
          Debrief
        </button>
        <button
          className={`tab-link ${tab === "loot" ? "active" : ""}`}
          onClick={() => setTab("loot")}
        >
          Loot
        </button>
        <button
          className={`tab-link ${tab === "map" ? "active" : ""}`}
          onClick={() => setTab("map")}
        >
          Map
        </button>
        <button
          className={`tab-link ${tab === "voice" ? "active" : ""}`}
      ...
      {tab === "events" && (
        <div>
          ...
          {events && (
            <SessionEventList events={events} onDelete={handleDeleteEvent} />
          )}
        ...
        <div className="summary-tab">
            ...
            <button
              ...
              title={
                !events || events.length === 0
                  ? "Add events before generating a summary"
                  : undefined
              }
            >
              {summaryLoading ? (
          ...
          {!summary && !summaryLoading && !summaryError && (
            <p className="empty-state">
              Click "Generate Summary" to create an AI-powered session recap.
            </p>
          )}
        </div>
      )}
        ...
        <div className="debrief-tab">
          <div className="section-header">
            <div>
              ...
              >
                Get a structured post-session analysis covering what went well,
                what was confusing, open plot threads, and next session
                suggestions.
              </p>
            </div>
            <button
          ...
          {showDebriefPanel && (
            <div className="debrief-panel" style={{ marginTop: "1rem" }}>
              ...
              {debriefText && (
                <div
                  className="debrief-content card"
                  style={{
                    padding: "1.5rem",
          ...
          {!showDebriefPanel && !debriefLoading && (
            <p className="empty-state">
              Click "Generate Debrief" to receive an AI-powered post-session
              coaching report.
            </p>
          )}
        </div>
      ...
      {tab === "voice" && (
        <div className="voice-tab">
          ...
          <p style={{ color: "#aaa", fontSize: 13, marginBottom: 12 }}>
            Speak session notes. Save as an event to add them to the session
            log.
          </p>
          <VoiceNoteCapture
